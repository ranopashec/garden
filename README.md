...
**Логика доступа:**
- `access: "public"` - статья доступна всем пользователям
- `access: "private"` - статья доступна только для пользователей, чьи Telegram ID есть в базе данных Supabase с актуальным сроком действия
- `access: "none"` или без поля `access` - статья не показывается никому (показывается сообщение о запрете доступа)
...
**Скрытая статья (не показывается никому):**
```yaml
---
title: "Скрытая статья"
access: "none"
permalink: "Скрытая"  # Без слэшей
---
```
