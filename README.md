...
**Логика доступа:**
- `access: private` - статья доступна только для пользователей, чьи никнеймы есть в базе данных Supabase
- `access: public` или без указания - статья доступна всем

**Настройка Supabase:**
Для работы приватных статей необходимо настроить Supabase. Подробная инструкция в файле [SUPABASE_SETUP.md](./SUPABASE_SETUP.md)

### 3. Настройка Supabase (для приватных статей)

Для работы приватных статей необходимо:
1. Создать проект в Supabase
2. Создать таблицу `allowed_users` с колонкой `username`
3. Добавить переменные окружения в Vercel:
   - `SUPABASE_URL` - URL вашего проекта Supabase
   - `SUPABASE_ANON_KEY` - публичный API ключ Supabase

Подробная инструкция: [SUPABASE_SETUP.md](./SUPABASE_SETUP.md)

### 4. Настройка в Vercel

1. Убедитесь, что проект использует Node.js 20.x
2. Добавьте переменные окружения для Supabase (см. выше)
...
garden/
├── api/
│   └── check-access.js         # API endpoint для проверки доступа через Supabase
├── src/
│   └── site/
│       ├── _includes/
│       │   ├── components/
