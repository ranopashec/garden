...
- ✅ Приватные статьи (доступ по username из файла accepted.md)
...
ACCEPTED_USERS=username1,username2,username3
```
...
Доступ к приватным статьям настраивается через переменную окружения `ACCEPTED_USERS`:
- Список username пользователей Telegram через запятую
- Без символа @ в начале
- Username проверяется без учета регистра
- Можно добавлять/удалять пользователей, обновляя переменную окружения

**Пример:**
```
ACCEPTED_USERS=lidevty_manager,ranopashec,another_user
```
...
1. Добавьте переменные окружения в настройках проекта Vercel:
   ...
   - `ACCEPTED_USERS` - список разрешенных username через запятую (обязательно)

2. Убедитесь, что функция `api/check-access` использует Node.js 20.x

**Примечание:** В будущем можно легко заменить проверку через переменную окружения на вызов внешнего API для автоматического управления пользователями.
